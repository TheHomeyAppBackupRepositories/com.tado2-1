...
module.exports = class TadoOAuth2Client extends OAuth2Client {
  ...
  async _onHandleGetTokenByErrorGeneric({ response }) {
    return super._onHandleGetTokenByErrorGeneric({ response })
      .catch(error => {
        if (error.message && error.message.includes('refresh token')) throw new Error(this.homey.__('errors.invalid_refresh_token'));
